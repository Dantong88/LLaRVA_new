# this file is to creare a rpt fine-tuning subset
...
if __name__ == '__main__':
    ...
    # Add arguments
    parser.add_argument('--subset_name', default='berkeley_rpt_converted_externally_to_rlds_new', type=str, help='An input name')
    parser.add_argument('--episode_num', default=50, type=int, help='An input name')
    parser.add_argument('--task', default='pick blue cube', type=str, help='An input name')
    parser.add_argument('--task_info', default='/home/niudt/LLaVA/process_dataset/finetune/rpt_task_list.json', type=str)
    ...
    task = args.task
    task_info = json.load(open(args.task_info))
    ...
    for subset in [subset_name]:
        ...
        train_start_idx = 1
        val_start_idx = 1
        eps_list = task_info[task]
        random.shuffle(eps_list)
        train_list = eps_list[:episode_num]
        val_list = eps_list[episode_num:]
        ...
        save_root = './finetuning_anns'
        os.makedirs(save_root, exist_ok=True)
        train_save_path = os.path.join(save_root, '{}-{}-{}-train.json'.format(subset, task, train_start_idx - 1))
        with open(train_save_path, 'w') as file:
            json.dump(train_split, file)

        val_save_path = os.path.join(save_root, '{}-{}-{}-val.json'.format(subset, task, val_start_idx - 1))
