...
def create_split(_root, subset, eps_list, start_idx):
    ...
    annotation_action_root = os.path.join(_root, 'actions')
    annotation_trajectory_root = os.path.join('/home/niudt/dataset', 'trajectories')
    ...
    for eps in tqdm(eps_list):
        ...
        trajectory_path = os.path.join(annotation_trajectory_root, subset, 'episode_{}.json'.format(eps))
        if not os.path.exists(trajectory_path):
            continue
        tjs = json.load(open(trajectory_path))
        images = os.listdir(image_path)
        for idx, image in enumerate(images):
            # add the trj
            tj = find_tj(tjs, idx, 'ann', 1)
            if len(tj) == 0:
                continue
            _img_path = os.path.join(image_path, image)
            if subset == 'berkeley_rpt_converted_externally_to_rlds':
            ...
            # 'interpretation':,same
            # 'prev_actions':, same
            # 'pred_actions':, [[i+1 step], [i+2 step], ...[i+c step]],
            # 'num_prediction_steps': c (10-16)
            # 'instruction':
            # }
            ...
            human = {}
            human[
                'value'] = '<image>\nYou are a {} robot using the {}. The task is \"{}\", and the previous five (including current) steps is {}, can you predict the action of the next step and the trajectories of the end effector?'.format(
                robot_type, control_type, instruction, prev_action)
            human['from'] = 'human'
            gpt = {}
            gpt['from'] = 'gpt'
            gpt['value'] = 'The next step: {}. The trajectories of the end effector: {}'.format(pred_actions, tj)
...
if __name__ == '__main__':
    ...
    # Add arguments
    parser.add_argument('--idx', default=0, type=int, help='An input name')
    ...
    subsets = [sb for sb in subsets if not sb in DOWNSAMPLE]
    ann_count = 1
    new_annotation = []
    Enable_Action = True
    train_start_idx = 1
